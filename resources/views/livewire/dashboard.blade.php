        ...
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-4">
                ...
                <div
                    ...
                    <div class="text-xs text-slate-400 mt-1">Servers monitored</div>
                </div>
                ...
                <div
                    ...
                    <div class="text-xs text-indigo-200 mt-1">{{ $activeCount }} active monitored</div>
                </div>
                ...
                <div
                    ...
                    <div class="text-xs text-emerald-100 mt-1">Services Operational</div>
                </div>
                ...
                <div
                    ...
                    <div class="text-xs text-red-100 mt-1">
                        {{ $downCount > 0 ? 'Services Down' : 'All Services Operational' }}
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="text-4xl font-black">
                        @if ($avgResponseTime)
                            {{ $avgResponseTime > 1000 ? round($avgResponseTime / 1000, 3) : $avgResponseTime }}
                            <span
            ...
            {{-- ===================== ROW 3.5 — SSH SERVER PERFORMANCE DIAGNOSTICS ===================== --}}
            @if (count($cpuChartData['datasets']) > 0)
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6" x-data="{ activeTab: 'cpu' }">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                        <div>
                            <h3 class="font-bold text-gray-800 text-lg">SSH Server Performance Diagnostics</h3>
                            <p class="text-xs text-gray-400 mt-0.5">Real-time CPU, Memory, and Disk utilization over
                                the last 24 hours</p>
                        </div>

                        <!-- Tabs toggles -->
                        <div
                            class="flex items-center gap-1.5 bg-gray-50 border border-gray-200 rounded-xl p-1 self-start sm:self-center">
                            <button @click="activeTab = 'cpu'"
                                :class="activeTab === 'cpu' ? 'bg-white text-indigo-600 shadow-sm' :
                                    'text-gray-500 hover:text-gray-800'"
                                class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all">
                                CPU Usage
                            </button>
                            <button @click="activeTab = 'ram'"
                                :class="activeTab === 'ram' ? 'bg-white text-indigo-600 shadow-sm' :
                                    'text-gray-500 hover:text-gray-800'"
                                class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all">
                                Memory Usage
                            </button>
                            <button @click="activeTab = 'disk'"
                                :class="activeTab === 'disk' ? 'bg-white text-indigo-600 shadow-sm' :
                                    'text-gray-500 hover:text-gray-800'"
                                class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all">
                                Disk Space
                            </button>
                    ...
                    <div class="w-full relative min-h-[300px]">
                        <div x-show="activeTab === 'cpu'" class="h-[300px]">
                            <canvas id="perfCpuChart" class="w-full h-full"></canvas>
                        </div>
                        <div x-show="activeTab === 'ram'" class="h-[300px]" style="display: none;"
                            :style="activeTab === 'ram' ? 'display: block;' : 'display: none;'">
                            <canvas id="perfRamChart" class="w-full h-full"></canvas>
                        </div>
                        <div x-show="activeTab === 'disk'" class="h-[300px]" style="display: none;"
                            :style="activeTab === 'disk' ? 'display: block;' : 'display: none;'">
                            <canvas id="perfDiskChart" class="w-full h-full"></canvas>
                        </div>
                    </div>
                </div>
            @endif
...
@push('scripts')
    <script>
        ...
        // ---- SSH CPU Performance Chart ----
        if (document.getElementById('perfCpuChart')) {
            new Chart(document.getElementById('perfCpuChart'), {
                type: 'line',
                data: @js($cpuChartData),
                options: {
                    responsive: true,
        ...
        // ---- SSH RAM Performance Chart ----
        if (document.getElementById('perfRamChart')) {
            new Chart(document.getElementById('perfRamChart'), {
                type: 'line',
                data: @js($ramChartData),
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    scales: {
        ...
        // ---- SSH Disk Performance Chart ----
        if (document.getElementById('perfDiskChart')) {
            new Chart(document.getElementById('perfDiskChart'), {
                type: 'line',
                data: @js($diskChartData),
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    scales: {
